...
export function calculateOrbitPath(satelliteMesh) {
  ...
  const satRecord = satelliteRecords.find(
    (rec) => rec.name === satelliteMesh.userData.satelliteName,
  );
  ...
  const orbitPeriodMinutes =
    CONFIG.ORBIT_PERIODS[orbitType] || CONFIG.ORBIT_PERIODS.LEO;
  ...
  if (points.length > 0) {
    ...
    console.log(
      `Calculated orbit path: ${points.length} points for ${satelliteMesh.userData.satelliteName} (${orbitType})`,
    );
  }
...
export function showOrbitPath(satelliteMesh, isSelected = false) {
  ...
  console.log(
    "showOrbitPath called for:",
    satelliteMesh.userData.satelliteName,
    "isSelected:",
    isSelected,
  );
