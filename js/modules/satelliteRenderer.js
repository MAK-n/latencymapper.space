...
export function renderSatellites(scene, satelliteRecords, satelliteTargetPositions) {
    ...
    satelliteRecords.forEach((satRecord, index) => {
        console.log(`[SatelliteRenderer] Processing satellite ${index}:`, satRecord.name);
        
        const position = satelliteTargetPositions[index];
        if (!position) {
            console.warn(`[SatelliteRenderer] No position for satellite ${index}:`, satRecord.name);
            return;
        }
        
        console.log(`[SatelliteRenderer] Position for ${satRecord.name}:`, position);
        ...
        console.log(`[SatelliteRenderer] Orbit type for ${satRecord.name}:`, orbitType);
        
        // Check if this is a custom satellite
        const isCustom = satRecord.isCustom === true;
        console.log(`[SatelliteRenderer] Is custom satellite for ${satRecord.name}:`, isCustom);
        
        // Create geometry and material
        const geometry = createSatelliteGeometry();
        const material = createSatelliteMaterial(orbitType, isCustom);
        console.log(`[SatelliteRenderer] Created geometry and material for ${satRecord.name}, color:`, isCustom ? 'RED' : orbitType);
        ...
        console.log(`[SatelliteRenderer] 3D position for ${satRecord.name}:`, position3D);
        ...
        console.log(`[SatelliteRenderer] Mesh userData for ${satRecord.name}:`, mesh.userData);
        
        scene.add(mesh);
        console.log(`[SatelliteRenderer] Added mesh to scene for ${satRecord.name}, scene children count:`, scene.children.length);
        
        satelliteMeshes.push(mesh);
        renderedCount++;
        console.log(`[SatelliteRenderer] ✓ Rendered ${satRecord.name}, total meshes:`, satelliteMeshes.length);
    });
    
    console.log(`[SatelliteRenderer] ✓ Rendered ${renderedCount} new satellites, total ${satelliteMeshes.length} satellites on globe`);
