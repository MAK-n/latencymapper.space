...
export function initAddSatelliteModal() {
  ...
  if (form) {
    ...
    if (loadIssBtn) {
      loadIssBtn.addEventListener("click", () => {
        ...
        if (tle1Input && tle2Input) {
          tle1Input.value =
            "1 25544U 98067A   25346.15413386  .00014398  00000-0  26408-3 0  9999";
          tle2Input.value =
            "2 25544  51.6307 145.3792 0003284 237.6977 122.3694 15.49529746542766";
    ...
    form.addEventListener("submit", async (e) => {
      ...
      console.log("[CustomSatellite] Form values:", {
        name,
        tle1Length: tle1.length,
        tle2Length: tle2.length,
      });

      if (!name || !tle1 || !tle2) {
        console.error("[CustomSatellite] Empty form values");
        alert(
          "Please fill all required fields (Name, TLE Line 1, and TLE Line 2).",
        );
        return;
      }
      ...
      try {
        ...
        console.log(
          "[CustomSatellite] Propagating position at time:",
          currentTime,
        );
        ...
        console.log(
          "[CustomSatellite] Added to records, new count:",
          records.length,
        );
        ...
        if (window.TREE) {
          ...
          console.log(
            "[CustomSatellite] New meshes created:",
            meshesAfter - meshesBefore,
          );
          ...
          console.log("[CustomSatellite] Updated cached arrays:", {
            ...
            targetPositions:
              window.getSatelliteTargetPositions?.()?.length || "N/A",
            currentPositions:
              window.getSatelliteCurrentPositions?.()?.length || "N/A",
          });
          ...
          if (lastMesh && lastMesh.userData.satelliteName === name) {
            console.log(
              "[CustomSatellite] ✓ New mesh confirmed in cached array at index:",
              allMeshes.length - 1,
            );
          ...
          console.log(
            "[CustomSatellite] ✓ Custom satellite rendered and added to scene",
          );
        } else {
          console.error(
            "[CustomSatellite] ERROR: window.TREE (scene) is not available!",
          );
        }
        ...
        form.reset();
        console.log(
          "[CustomSatellite] ✓ Custom satellite added successfully:",
          name,
        );
      } catch (err) {
