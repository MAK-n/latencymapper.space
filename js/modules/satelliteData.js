...
export async function loadSatelliteData() {
    try {
        ...
        console.log(`✓ Loaded ${satellites.length} satellites (limited from ${allSatellites.length} total)`);
        return satellites;
...
export function initializeSatelliteRecord(satellite) {
    try {
        const satrec = window.satellite.twoline2satrec(satellite.tle1, satellite.tle2);
        
        if (satrec.error) {
            console.warn(`Error initializing satellite ${satellite.name}:`, satrec.error);
            return null;
        }
...
export function propagateSatellitePosition(satRecord, time) {
    try {
        const positionAndVelocity = window.satellite.propagate(satRecord.satrec, time);
