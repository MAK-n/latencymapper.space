...
import {
  ...
  getStationMeshes,
  clearStationMeshes,
} from "./modules/groundStationRenderer.js";
import {
  setupStationInteraction,
  setupInfoPanelHandlers,
} from "./modules/groundStationInteraction.js";
import {
  loadSatelliteData,
...
import {
  renderSatellites,
  getSatelliteMeshes,
} from "./modules/satelliteRenderer.js";
import {
...
async function init() {
  ...
  if (satellites.length > 0) {
    ...
    if (records.length > 0) {
      ...
      import("./modules/heatmapOverlay.js").then((mod) => {
        mod.initHeatmapOverlay(scene);
      });
...
window.addEventListener("activate-globe-click-mode", () => {
  ...
  if (typeof camera !== "undefined" && typeof renderer !== "undefined") {
    ...
    if (scene && scene.children) {
      globe = scene.children.find(
        (obj) => obj.name === "Earth" || obj.userData.isEarth,
      );
    }
...
function animate() {
  ...
  // Sync heatmap rotation with Earth (non-blocking)
  if (window.syncHeatmapRotation) {
    window.syncHeatmapRotation();
  }
...
/**
 * Trigger an immediate render (useful for forcing UI updates after async operations)
 */
window.triggerRender = function () {
  if (renderer && scene && camera) {
    // Force a render in the next frame
